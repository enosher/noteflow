...
5. [Features](#features)
6. [Spaced Repetition](#spaced-repetition)
...
12. [Why NoteFlow Instead of ChatGPT](#why-noteflow-instead-of-chatgpt)
13. [Tech Stack](#tech-stack)
14. [Software Engineering Practices](#software-engineering-practices)
15. [Project Management](#project-management)
16. [Testing](#testing)
17. [User Testing](#user-testing)
...
A toggle in the nav bar switches the entire app between light and dark themes.
...
*Figure 3: ER Diagram, exported from Supabase's schema visualiser*
...
That interpretability paid off during M3 manual QA. The demo account's recommendation surfaced a Polymorphism question over a weaker Recursion topic, contradicting the account's own narrative. Because every term is a named, inspectable function, the cause was traceable rather than a black-box anomaly: the seed data's attempt timestamp sat just outside the `recency_boost` window, zeroing out a term the narrative assumed would fire. The formula was working exactly as coded - only the seed data's timestamp didn't match its own story. See Known Issue `REC-01-MISMATCH` in `docs/manual-test-log.md`.
...
Testers 3 and 5 hit the same root cause, traced directly rather than guessed at: `generateQuestionDrafts()` only counts a note as usable if its `content` field has real text, and a note created by uploading a file (Tester 5's PDF) leaves `content` empty, since the app has no PDF-to-text extraction step. Auditing the seed data found the same gap independent of any upload - half the seeded topics in every module had zero notes, so generation would have failed for Tester 3 regardless of Gemini. Fixed for the demo/testing path: every previously note-less topic in `lib/seed-data.ts` and `lib/seed-demo-data.ts` now has a real markdown note, locked in by a new test (`lib/seed-data.test.ts`). A real user who only uploads files without typing any notes will still hit this - tracked in `docs/m3-ui-fixes-plan.md`.

The remaining findings were fixed directly in code. The module page's topic list now has the same chevron and hover-underline treatment used elsewhere, closing the note-creation discoverability gap. The dashboard's duplicate "Weak topics" labeling is gone: the non-interactive KPI stat is now "Weak," and "Recommended next" got more visual weight so it reads as a heading. Quiz questions and MCQ options now shuffle on every attempt (`lib/shuffle.ts`, with its own tests), addressing the "remembering the ABCs" concern. Review's MCQ cards show right/wrong feedback and the correct answer before advancing, and the session-end screen shows a real score instead of just a count. The concept graph's prerequisite-removal control is now a persistent marker on every connector line instead of a hover-only target. Breadcrumbs now extend to the note (and its subtopic, if any) instead of stopping at the topic.

A full persistent sidebar - the specific "one change" Tester 4 said would make them actually use NoteFlow - is now built, not just scoped (`components/Sidebar.tsx`, `components/SidebarNav.tsx`). It renders a module → topic → subtopic → note tree on every page, auto-expands whatever branch the current page is in, and highlights the active item, so switching notes no longer means navigating back through the topic page. It's desktop-only (`hidden lg:block`) for this pass; a mobile version (collapsible drawer, hamburger toggle) wasn't attempted.
...
- **PDF / file text extraction for AI question generation.** Uncovered during M3 user testing (see [User Testing](#user-testing) and Known Limitations above): a file-only note with no typed or pasted text never contributes content to `generateQuestionDrafts()`, since there's no text-extraction step for uploads - a user who only uploads PDFs will hit "no usable note content." Two options were scoped but not built: add server-side PDF-to-text extraction (e.g. `pdf-parse`), which is real work given variable parsing quality and serverless time/memory limits; or leave file-only notes unsupported and say so next to the upload field, not just here. Deprioritised given the M3 deadline in favour of fixing the demo-facing version of the same gap (see Changes Made under Milestone 3 User Testing).
